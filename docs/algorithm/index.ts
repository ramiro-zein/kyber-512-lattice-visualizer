...
const encoding = {
  alias: "enc",
  choices: ["ascii", "utf8", "utf-8", "utf16le", "utf-16le", "ucs2", "ucs-2", "base64", "base64url", "latin1", "binary", "hex"] as const,
  default: "hex",
} as const;
...
const argv = yargs(hideBin(process.argv))
  .command({
    command: "prng", // Pseudo Random Number Generator
    describe: "Generate random number",
    handler: ({ type, size, min, max, encoding }) => {
      console.log("Output: " + prng(type, size, min, max, encoding));
    },
    builder: {
      type: {
